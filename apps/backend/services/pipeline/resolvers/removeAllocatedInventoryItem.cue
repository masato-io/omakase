...
removeAllocatedInventoryItemOutput : {
    Name: "removeAllocatedInventoryItemOutput",
    Fields: [
...
removeAllocatedInventoryItem : pipeline.#Resolver & {
    Authorization: pipeline.#AuthInsecure,
    Name:          "removeAllocatedInventoryItem",
    Description: """
        Consumes a specified quantity of an inventory item.
    """,
    Response: {Type: removeAllocatedInventoryItemOutput},
    Inputs: [
        {
            Name:     "input",
            Required: true,
            Type: {
                Name: "removeAllocatedInventoryItemInput",
                Fields: [
            ...
            PostHook : common.#Script & {
				...
				(() => {
					...
					if (item.woAllocatedQuantity < requestedQuantity) {
						return {
							success: false,
							message: 'Insufficient woAllocatedQuantity in inventory.'
						};
					}
					return {
						success: true,
