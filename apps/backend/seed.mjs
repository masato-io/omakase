  ...
  do {
    ...
    try {
      ...
      if (error.message.includes("token is expired")) {
        ...
        await $$`tailorctl config switch mo`;
        continue;
      }
...
async function findCueFiles(dir) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (
      ...
      fullPath.endsWith(`${path.sep}utils`)
    ) {
...
async function generateTestData(seedFilePath, outputPath) {
  ...
  const outputFilePath =
    outputPath ||
    path.join(
      path.dirname(seedFilePath),
      `${path.basename(seedFilePath, ".json")}_testData.json`
    );
...
async function seed() {
  ...
  for (const file of cueFiles) {
    ...
    try {
      await execAsync(`cue eval ${filePath} --out json > ${jsonPath}`, {
        shell: true,
      });
      if (generateTestDataArg) {
        ...
        const testDataPath = path.join(
          generateTestDataArg,
          `${path.basename(filePath, ".cue")}.json`
        );
